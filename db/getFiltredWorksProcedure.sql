CREATE OR REPLACE FUNCTION get_filtred_data(
    IN period_start_in DATE DEFAULT NULL,
    IN period_end_in DATE DEFAULT NULL,
...
BEGIN
    ...
    WHERE
        ...
        AND (object_name_in IS NULL OR w.object_name = object_name_in)
        AND (work_type_in IS NULL OR w.work_type = work_type_in)
    ORDER BY
        w.work_date;
END;
$$ LANGUAGE plpgsql;